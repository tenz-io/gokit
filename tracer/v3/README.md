...
func main() {
	...
	// 1. 写入请求 ID，供日志、下游调用透传使用
	ctx = tracer.WithRequestID(ctx, "req-123")
	fmt.Println(tracer.RequestIDFromCtx(ctx)) // req-123

	// 未设置时自动生成（去掉短横线的 UUID，32 个十六进制字符）
	fmt.Println(tracer.RequestIDFromCtx(context.Background()))
	...
	// 3. 把请求头里的字符串直接解析成 Flag（替代手写 switch）
	parsed := tracer.ParseFlag("debug|stress|shadow")
	ctx = tracer.WithFlag(ctx, parsed)
...
| 字符串解析为标志位 | `ParseFlag("debug\|shadow")` 把 `\|` 分隔的字符串解析成 `Flag`，大小写不敏感、未知项忽略，替代调用方手写 switch |
| 标志位转字符串 | `Flag.String()` 返回 `debug\|shadow` 形式的可读字符串；`Flag.Names()` 返回各标志位名字的切片 |

## API 速查

| 符号 | 说明 |
| --- | --- |
| `type Flag int8` | 流量模式标志位的位掩码类型（`int8`，更小） |
...
| `(Flag) String() string` | 返回 `debug\|shadow` 形式的可读字符串，无标志返回 `none` |
| `(Flag) Names() []string` | 返回所有已置位标志的名字切片（`flagTable` 顺序），无标志返回 `nil` |
| `ParseFlag(s string) Flag` | 把 `\|` 分隔字符串解析成 `Flag`，大小写不敏感、未知项忽略，空串返回 `FlagNone` |
...
| `Flag` 底层类型 | `int` | `int8`（位掩码仅需 3 个标志，更小） |
| context key | 字符串常量（`"_flag_ctx_key"` 等） | 类型化空结构体（零碰撞、无字符串比较） |
| 字符串互转 | 无（调用方各自手写 switch，如 `ginext/utils.go` 的 `getTracerFlag`） | `ParseFlag` / `Flag.String()` 收口到本模块 |
| 预留位渲染 | 不适用 | 未知位（`allFlagBits` 之外）在 `String()` 中被丢弃而非 panic |
