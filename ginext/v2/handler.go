...
type (
	TracerRpcInterceptor    struct{ interceptor RpcInterceptor }
	MetricsRpcInterceptor    struct{ interceptor RpcInterceptor }
	TrafficRpcInterceptor    struct{ interceptor RpcInterceptor }
	SlogLogRpcInterceptor    struct{ interceptor RpcInterceptor; threshold time.Duration }
	PanicRecoverRpcInterceptor struct{ interceptor RpcInterceptor }
	handlerRpcInterceptor    struct{}
)

type NewRpcInterceptor func(interceptor RpcInterceptor) RpcInterceptor
...
func (t *TracerRpcInterceptor) Intercept(ctx context.Context, req any, handler RpcHandler) (resp any, err error) {
	...
	ctx = logger.WithLogger(ctx,
		logger.WithRequestID(meta.RequestID).
			With("url", meta.Path, "entry_cmd", meta.Cmd, "request_flag", meta.RequestFlag),
	)
...
func (m *MetricsRpcInterceptor) Intercept(ctx context.Context, req any, handler RpcHandler) (resp any, err error) {
	...
	defer func() { rec.EndWithCode(getErrCode(err)) }()
...
func (t *TrafficRpcInterceptor) Intercept(ctx context.Context, req any, handler RpcHandler) (resp any, err error) {
	...
	rec := entry.StartTraffic(meta.Cmd)
	if rec != nil {
		defer func() {
			rec.End(resp, getErrCode(err))
		}()
	}
...
func NewSlogLogRpcInterceptor(interceptor RpcInterceptor) RpcInterceptor {
	return &SlogLogRpcInterceptor{interceptor: interceptor, threshold: 5 * time.Second}
}
...
func (s *SlogLogRpcInterceptor) Intercept(ctx context.Context, req any, handler RpcHandler) (resp any, err error) {
	...
	defer func() {
		if duration := time.Since(start); s.threshold > 0 && duration > s.threshold {
			le.With("duration", duration, "threshold", s.threshold,
				"method", meta.Method, "client_ip", meta.ClientIP,
				"command", meta.Cmd, "err_code", getErrCode(err),
				"err_msg", getErrMsg(err)).Warn("slow log")
		}
	}()
...
func (p *PanicRecoverRpcInterceptor) Intercept(ctx context.Context, req any, handler RpcHandler) (resp any, err error) {
	defer func() {
		if r := recover(); r != nil {
			...
			logger.FromContext(ctx).With("panic", fmt.Sprintf("%s", r)).Error("panic recovery")
...
func newHandlerRpcInterceptor() RpcInterceptor { return &handlerRpcInterceptor{} }
