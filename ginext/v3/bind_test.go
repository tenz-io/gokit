...
func Test_tryBindMultipart(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotMultipart, err := tryBindMultipart(tt.c, tt.ptr)
			t.Logf("gotMultipart: %v, err: %v", gotMultipart, err)
			if !tt.wantErr(t, err, fmt.Sprintf("tryBindMultipart(%v, %v)", tt.c, tt.ptr)) {
				return
			}
			assert.Equalf(t, tt.wantMultipart, gotMultipart, "tryBindMultipart(%v, %v)", tt.c, tt.ptr)
		})
	}
}
